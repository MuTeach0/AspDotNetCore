...
using M05.BuildingRESTFulApi.Models;
using M05.BuildingRESTFulApi.Requests;
using M05.BuildingRESTFulApi.Responses;
using Microsoft.AspNetCore.Http.HttpResults;
using Microsoft.AspNetCore.JsonPatch;
using Newtonsoft.Json;

namespace M05.BuildingRESTFulApi.Endpoints;
...
public static class ProductEndpoints
{
	...
	private static IResult HeadProduct(Guid productId, ProductRepository repository) =>
		repository.ExistsById(productId) ? Results.Ok() : Results.NotFound();
	...
	private static IResult GetPaged(ProductRepository repository, int page = 1, int pageSize = 10)
	{
		...
		var pageResult = PageResult<ProductResponse>.Create(ProductResponse.FromModels(products), totalCount, page, pageSize);
	...
	private static IResult CreateProductReview(Guid productId, CreateProductReviewRequest request, ProductRepository repository)
	{
	...
	private static async Task<IResult> Patch(Guid productId, ProductRepository repository,
	...
	private static IResult GetRedirect() => Results.Redirect("/API/PRODUCTS/TEMP-PRODUCTS");
	private static IResult TempProducts() => Results.Ok(new { message = "You're in the temp endpoint. Chill." });
	private static IResult GetPermanentRedirect() => Results.Redirect("/API/PRODUCTS/PRODUCT-CATALOG", permanent: true);
	private static IResult Catalog() => Results.Ok(new { message = "This is the permanent new location" });
	// private static IResult GetLegacyProducts() => Results.Ok(new { message = "You're in the legacy products endpoint." });
}